...
function UdinTable({ rows, onDelete }: { rows: FirmUdinRow[]; onDelete: (u: FirmUdinRow) => void }) {
  return (
    <table className="w-full min-w-[820px] text-left text-xs">
      ...
      <tbody>
        {rows.length === 0 ? (
          <EmptyRow cols={7} text="No UDINs recorded yet." />
        ) : (
          rows.map((u) => (
            <tr key={u.id} className="border-b border-white/[0.04] last:border-0 hover:bg-white/[0.02]">
              ...
              {/* Which signed document this number belongs to — the question the
                  register exists to answer, and could not until now. A UDIN
                  recorded straight into the register still has no filing, and
                  says so rather than looking complete. */}
              <td className="max-w-44 truncate px-3 py-2.5 text-muted-foreground">
                {u.filing
                  ? `${u.filing.filingType}${u.filing.period ? ` · ${u.filing.period}` : ""}`
                  : <span className="text-amber-400/80">Not linked to a filing</span>}
              </td>
